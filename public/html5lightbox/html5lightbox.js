...
function loadHtml5LightBox(jsFolder) {
        ...
        $.fn.html5lightbox = function (options) {
            ...
            var ELEM_TYPE = 0, ELEM_HREF = 1, ELEM_TITLE = 2, ELEM_GROUP = 3, ELEM_WIDTH = 4, ELEM_HEIGHT = 5,
                ...
                ELEM_ORIGINALWIDTH = 11, ELEM_ORIGINALHEIGHT =
                    12, ELEM_SOCIALMEDIA = 13, ELEM_WEBLINK = 14, ELEM_WEBLINKTARGET = 15, ELEM_WEBLINKTEXT = 16,
                ELEM_MEDIUMWIDTH = 17, ELEM_MEDIUMHEIGHT = 18, ELEM_SMALLWIDTH = 19, ELEM_SMALLHEIGHT = 20;
            ...
            inst.readData = function () {
                inst.each(function () {
                    ...
                    if (this.nodeName.toLowerCase() ==
                        "a" || this.nodeName.toLowerCase() == "area") inst.readNodeData(self); else self.find("a,area").each(function () {
                ...
                if (inst.options.showsocial) {
                    ...
                    $(".html5-social-btn", inst.$lightbox).click(function () {
                        ...
                        if (inst.currentElem[ELEM_TYPE] == 0) shareMedia = inst.absoluteUrl(inst.currentElem[ELEM_HREF]);
                            ...
                            if (lightboxLink.length > 0) if (lightboxLink.data("shareimage") && lightboxLink.data("shareimage").length > 0) shareMedia = inst.absoluteUrl(lightboxLink.data("shareimage")); else if (lightboxLink.data("thumbnail") && lightboxLink.data("thumbnail").length > 0) shareMedia = inst.absoluteUrl(lightboxLink.data("thumbnail"));
                            else {
                        ...
                        if (!shareTitle) shareTitle = inst.options.sharetextprefix; else shareTitle = inst.html2Text(shareTitle);
                        if ($(this).hasClass("html5-social-facebook")) window.open("https://www.facebook.com/sharer/sharer.php?u=" +
                            ...
                            "true" : "false"), "_blank"); else if ($(this).hasClass("html5-social-email")) window.open("mailto:?subject=" + encodeURIComponent(shareTitle) + "&body=" + encodeURIComponent(inst.options.sharetextprefix + " " + shareUrl));
                        return false
                    })
                }
            ...
            inst.showNavigation = function () {
                ...
                if (inst.options.shownavcontrol) {
                    ...
                    $(".html5-nav-showcontrol").click(function () {
                        ...
                        if (inst.navvisible) {
                            ...
                            var navH = $(".html5-nav").height();
                            $(".html5-nav").css({top: winH + "px", bottom: "auto"}).animate({
                                top: winH -
                                    navH + "px"
                            }, function () {
                ...
                $navNext.click(function () {
                    ...
                    if (marginLeft >=
                        0) $navPrev.css({"background-position": "center left"}); else $navPrev.css({"background-position": "center right"});
                    $navList.animate({"margin-left": marginLeft})
                });
                var winWidth = $(window).width();
            ...
            inst.showVideo = function (elem) {
                ...
                inst.resizeLightbox(dataW, dataH, true, function () {
                    ...
                    if (inst.options.isIE678 || elem[ELEM_TYPE] == 8 || inst.options.isIE9 && inst.options.useflashonie9 ||
                        inst.options.isIE10 && inst.options.useflashonie10 || inst.options.isIE11 && inst.options.useflashonie11) isHTML5 = false; else if (inst.options.isMobile) isHTML5 = true; else if ((inst.options.html5player || !inst.options.flashInstalled) && inst.options.html5VideoSupported) {
                        isHTML5 = true;
                    ...
                    if (isHTML5) {
                        ...
                        if (inst.options.isFirefox ||
                            inst.options.isOpera) if (elem[ELEM_HREF_WEBM]) videoSrc = elem[ELEM_HREF_WEBM]; else if (elem[ELEM_HREF_OGG]) videoSrc = elem[ELEM_HREF_OGG];
                        ...
                        if (elem[ELEM_WEBLINK] && elem[ELEM_WEBLINK].length > 0) $(".html5-lightbox-video", inst.$image).css({cursor: "pointer"}).click(function () {
                            if (elem[ELEM_WEBLINKTARGET] && elem[ELEM_WEBLINKTARGET].length > 0) window.open(elem[ELEM_WEBLINK],
                                elem[ELEM_WEBLINKTARGET]); else window.open(elem[ELEM_WEBLINK])
                        })
                    } else {
            ...
            inst.calcElemSize = function (sizeObj, keepratio) {
                ...
                if (keepratio) {
                    ...
                    if (sizeObj.h > h0) sizeObj.h = h0; else if (inst.options.maxheight) sizeObj.h = h0;
            ...
            inst.adjustDataHeight = function () {
                ...
                if (inst.options.titlestyle == "bottom" || inst.options.titlestyle ==
                    "inside") inst.$lightboxBox.css({height: "auto"});
                    ...
                    inst.$elem.animate({opacity: 1}, {
                        duration: inst.options.transitionduration, always: function () {
                            ...
                            $(".html5-prev-inside, .html5-next-inside, .html5-prev-outside, .html5-next-outside",
                                inst.$lightbox).css({visibility: "visible"});
                            inst.$elem.removeClass("html5-elem-box-current").css({position: "relative", height: "100%"})
                        }
                    })
                }
    ...
    (function ($) {
        $.fn.lightboxHTML5VideoControls = function (skinFolder, parentInst, videoElem, hideControls, hidePlayButton, defaultVolume, fullscreenNativeControls, html5VideoNoDownload, skinImages, showsubtitle, vttline, showsubtitlebydefault) {
            ...
            $videoControls.css({
                display: "block",
                position: "absolute", width: "100%", height: BAR_HEIGHT, left: 0, bottom: 0, right: 0, margin: "0 auto"
            });
            ...
            if ($videoFSObj.get(0).requestFullscreen || $videoFSObj.get(0).webkitRequestFullScreen || $videoFSObj.get(0).mozRequestFullScreen || $videoFSObj.get(0).webkitEnterFullScreen || $videoFSObj.get(0).msRequestFullscreen) {
                var switchScreen = function (fullscreen) {
                    if (fullscreen) {
                        ...
                        else if ($videoFSObj.get(0).webkitRequestFullScreen) $videoFSObj.get(0).webkitRequestFullScreen(); else if ($videoFSObj.get(0).mozRequestFullScreen) $videoFSObj.get(0).mozRequestFullScreen(); else if ($videoFSObj.get(0).webkitEnterFullScreen) $videoFSObj.get(0).webkitEnterFullScreen();
            ...
            if (showsubtitle) {
                ...
                $videoCaption.css({
                    ...
                    if ($videoObj.get(0).textTracks) for (var i =
            ...
            if ($videoObj.get(0).volume === volume / 2 + 0.1) {
                ...
                $videoVolumeBarBg.css({
                    display: "block", position: "relative", width: 10, height: 68, margin: 7, cursor: "pointer",
                    "background-color": "#222"
                });
